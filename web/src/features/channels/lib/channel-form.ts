...
export const channelFormSchema = z
  .object({
    ...
    qca_environment_id: z.string().optional(), // QCA specific
...
export function transformChannelToFormDefaults(
  ...
  if (channel.settings) {
    try {
      ...
      qcaEnvironmentId = parsed.qca_environment_id || ''
  ...
  return {
    ...
    qca_environment_id: qcaEnvironmentId,
...
function buildSettingsJSON(formData: ChannelFormValues): string {
  ...
  // Add qca_environment_id for QCA channels
  if (formData.type === CHANNEL_TYPE_QCA && formData.qca_environment_id) {
    settingsObj.qca_environment_id = formData.qca_environment_id
  } else if ('qca_environment_id' in settingsObj) {
    delete settingsObj.qca_environment_id
  }
